...
df1 = pd.read_csv(file1, header=None, names=columns,
                  usecols=[0, 1, 2, 3, 4, 5])
df2 = pd.read_csv(file2, header=None, names=columns,
                  usecols=[0, 1, 2, 3, 4, 5])
...
df["date"] = df["date"].apply(lambda x: int(
    datetime.datetime.strptime(x, "%m/%d/%Y %H:%M:%S").timestamp()))

# Replace the Vector values with numeric values
df["vector"] = df["vector"].replace(
    {"http": 0, "email": 1, "device": 2, "logon": 3})
...
df.to_csv('data/scenario2_processed/df-export-allcolumns.csv')
...
df.rename(columns={"user_cat": "user", "source_cat": "source",
          "action_cat": "action"}, inplace=True)
print(df.head())
print(df.info())
